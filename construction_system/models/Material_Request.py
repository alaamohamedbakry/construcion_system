...
class ConstructionMaterialRequest(models.Model):
    ...
    line_ids = fields.One2many(
        "construction.material.request.line", "request_id", string="Material Lines"
    )
    ...
    stock_picking_ids = fields.One2many(
        "stock.picking", "material_request_id", string="Stock Pickings"
    )
    ...
    def action_approve(self):
        ...
        for request in self:
            ...
            if not request.project_id.site_location_id:
                raise UserError("Please set Site Location in Project")
            ...
            for line in request.line_ids:
                ...
                if available >= line.quantity:
                    stock_lines.append({"line": line, "qty": line.quantity})

                else:

                    if available > 0:
                        stock_lines.append({"line": line, "qty": available})

                    purchase_lines.append(
                        {"line": line, "qty": line.quantity - available}
                    )
                ...
                picking = self.env["stock.picking"].create(
                    {
                        ...
                        "location_dest_id": request.project_id.site_location_id.id,
                        "origin": request.name,
                        "material_request_id": request.id,
                        "cost_center_id": request.cost_center_id.id,
                        "analytic_account_id": request.cost_center_id.analytic_account_id.id,
                    }
                )
                    ...
                    self.env["stock.move"].create(
                        {
                            ...
                            "product_id": item["line"].product_id.id,
                            "product_uom_qty": item["qty"],
                            "product_uom": item["line"].product_uom_id.id,
            ...
            if purchase_lines:
                ...
                if not vendor:
                    raise UserError("Please set Vendor for products")
                    ...
                    self.env["purchase.order.line"].create(
                        {
                            ...
                            "product_uom": item["line"].product_uom_id.id,
                            "price_unit": item["line"].unit_price,
                            "name": item["line"].product_id.display_name,
