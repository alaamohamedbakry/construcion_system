...
class ConstructionTender(models.Model):
    ...
    def action_open_bid_file(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("construction_system.action_construction_bid_file")
        ...
        if len(bid_files) > 1:
            ...
            action["views"] = [(self.env.ref("construction_system.construction_bid_file_form").id, "form")]
    ...
    def action_open_boq(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("construction_system.action_construction_boq")
        action['context'] = {'default_tender_id': self.id}
